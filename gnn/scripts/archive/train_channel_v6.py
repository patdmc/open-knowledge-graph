#!/usr/bin/env python3
"""
Train ChannelNetV6 — 7 channels (+ Chromatin/Epigenetic meta-channel).
Tests whether the 7th channel adds predictive signal.

Usage:
    python3 -u -m gnn.scripts.train_channel_v6
    python3 -u -m gnn.scripts.train_channel_v6 --resume
"""
...
SAVE_BASE = os.path.join(GNN_RESULTS, "channelnet_v6_7ch")
...
def run_fold(fold_idx, train_idx, val_idx, data_dict, config):
    train_ds = ChannelDatasetV6(data_dict, indices=train_idx)
...
def main():
    ...
    print("=" * 65)
    print("  CHANNELNET V6 — 8 CHANNELS (+ ChromatinRemodel + DNAMethylation)")
    print(f"  hidden={CONFIG['hidden_dim']}, layers={CONFIG['cross_channel_layers']}, "
          f"batch={CONFIG['batch_size']}")
    print(f"  8 channels + 4 tiers + cancer type + clinical")
    print("=" * 65)
    ...
    print(f"  {N} patients, {n_ct} cancer types, {N_CHANNELS_V6} channels, {N_TIERS_V6} tiers")
    ...
    print(f"  vs ChannelNet V1 (6 ch):         0.620")
    print(f"  vs ChannelNet V2 (6+3t+ct):      0.676")
    print(f"  vs ChannelNet V5 (12 hub/leaf):   0.677")
    print(f"{'='*65}")

    summary = {
        "model": "ChannelNetV6_7channels",
        "config": CONFIG,
