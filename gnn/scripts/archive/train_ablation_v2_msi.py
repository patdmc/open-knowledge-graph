#!/usr/bin/env python3
"""
Ablation: V2 architecture (6 channels) + MSI/TMB clinical covariates.
Tests whether 8-channel expansion adds signal beyond MSI/TMB alone.

Usage:
    python3 -u -m gnn.scripts.train_ablation_v2_msi
"""
...
SAVE_BASE = os.path.join(GNN_RESULTS, "ablation_v2_msi")
...
def run_fold(fold_idx, train_idx, val_idx, data_dict, config):
    train_ds = ChannelDatasetV2MSI(data_dict, indices=train_idx)
...
def main():
    ...
    print("=" * 65)
    print("  ABLATION: V2 (6 CHANNELS) + MSI/TMB")
    ...
    print(f"\n{'='*65}")
    print(f"  ABLATION RESULTS ({elapsed/60:.1f} min)")
    print(f"  V2 (6ch) + MSI/TMB: {mean_ci:.4f} +/- {std_ci:.4f}")
    print(f"\n  vs V2 (6ch, no MSI/TMB):         0.6760")
    print(f"  vs V6c (8ch + MSI/TMB):          ~0.686")
    print(f"  Difference = 8ch contribution")
    print(f"{'='*65}")

    summary = {
        "model": "ChannelNetV2_MSI_Ablation",
        "config": CONFIG,
