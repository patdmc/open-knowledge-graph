#!/usr/bin/env python3
"""
Train ChannelNetV5 — 12-node hub/leaf model.
Tests whether within-channel graph structure (hub vs leaf) adds signal.

Usage:
    python3 -u -m gnn.scripts.train_channel_v5
    python3 -u -m gnn.scripts.train_channel_v5 --resume
"""
...
from gnn.data.channel_dataset_v5 import (
    build_hublf_features, HubLeafDataset, HUBLF_FEAT_DIM, N_NODES,
)
from gnn.models.channel_net_v5 import ChannelNetV5
...
SAVE_BASE = os.path.join(GNN_RESULTS, "channelnet_v5_hublf")
CHECKPOINT_PATH = os.path.join(SAVE_BASE, "checkpoint.json")


def collate_fn(batch):
    return {
        "hublf_features": torch.stack([b["hublf_features"] for b in batch]),
...
def run_fold(fold_idx, train_idx, val_idx, data_dict, config):
    train_ds = HubLeafDataset(data_dict, indices=train_idx)
    val_ds = HubLeafDataset(data_dict, indices=val_idx)
...
def main():
    ...
    print("=" * 60)
    print("  CHANNELNET V5 — HUB/LEAF SPLIT (12 nodes)")
    print(f"  hidden={CONFIG['hidden_dim']}, layers={CONFIG['cross_channel_layers']}, "
          f"batch={CONFIG['batch_size']}")
    print(f"  Features: 12 nodes (6 channels × hub/leaf) + cancer type + clinical")
    print("=" * 60)

    data_dict = build_hublf_features("msk_impact_50k")
    N = len(data_dict["times"])
    n_ct = len(data_dict["cancer_type_vocab"])
    CONFIG["n_cancer_types"] = n_ct
    print(f"  {N} patients, {n_ct} cancer types, {N_NODES} nodes")

    # Stratification
    event_arr = data_dict["events"].numpy().astype(int)
    node_count = (data_dict["hublf_features"][:, :, 0] > 0).sum(dim=1).numpy().clip(max=3)
    strat = event_arr * 4 + node_count
    ...
    print(f"  vs ChannelNet v1 (6 nodes):  0.620")
    print(f"  vs ChannelNet v2 (6+3+ct):   0.676")
    print(f"{'='*60}")

    summary = {
        "model": "ChannelNetV5_HubLeaf",
        "config": CONFIG,
