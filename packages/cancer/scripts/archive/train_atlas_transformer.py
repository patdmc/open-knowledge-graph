#!/usr/bin/env python3
"""
Train AtlasTransformer — transformer on the knowledge graph.

The atlas does the heavy lifting (node features = pre-computed HRs).
The transformer learns mutation interactions the additive model misses.

Usage:
    python -u -m gnn.scripts.train_atlas_transformer
"""
...
from gnn.models.atlas_transformer import AtlasTransformer
...
def main():
    ...
    print(f"\n{'='*50}", flush=True)
    print(f"ATLAS TRANSFORMER RESULTS", flush=True)
