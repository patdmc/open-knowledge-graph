#!/usr/bin/env python3
"""
Train ChannelNetV6c — 8 channels + MSI-aware clinical features.
Tests whether MSI context helps the model discount chromatin passengers.

Usage:
    python3 -u -m gnn.scripts.train_channel_v6c
    python3 -u -m gnn.scripts.train_channel_v6c --resume
"""
...
SAVE_BASE = os.path.join(GNN_RESULTS, "channelnet_v6c_msi")
...
def main():
    ...
    print("=" * 65)
    print("  CHANNELNET V6c — 8 CHANNELS + MSI-AWARE CLINICAL")
    print(f"  hidden={CONFIG['hidden_dim']}, layers={CONFIG['cross_channel_layers']}, "
          f"batch={CONFIG['batch_size']}")
    print(f"  Clinical: age, sex, msi_score, msi_high, tmb")
    print("=" * 65)
    ...
    n_msi = int(data_dict["msi_high"].sum())
    print(f"  {N} patients, {n_ct} cancer types, {n_msi} MSI-high")
    ...
    # Run folds 1,3,4,5,2 — see folds 3-5 early (where V6 dropped)
    FOLD_ORDER = [0, 2, 3, 4, 1]

    fold_results = {}
    if args.resume and os.path.exists(CHECKPOINT_PATH):
        with open(CHECKPOINT_PATH) as f:
            ckpt = json.load(f)
        for r in ckpt.get("fold_results", []):
            fold_results[r["fold"]] = r

    # Pre-compute all fold splits
    all_splits = list(skf.split(np.arange(N), strat))

    t0 = time.time()

    for fold_idx in FOLD_ORDER:
        if fold_idx in fold_results:
            print(f"\n  Fold {fold_idx+1} already done, skipping")
            continue
        ...
        with open(CHECKPOINT_PATH, "w") as f:
            json.dump({"fold_results": [fold_results[k] for k in sorted(fold_results)],
                        "config": CONFIG},
    ...
    fold_results_list = [fold_results[k] for k in sorted(fold_results)]
    c_indices = [r["c_index"] for r in fold_results_list]
    ...
    print(f"  vs ChannelNet V2 (6ch, 99g):     0.676")
    print(f"  vs ChannelNet V6 (8ch, 243g):    0.679")
    print(f"  vs Cox-Sage (RNA-seq):            0.63-0.72")
    print(f"{'='*65}")

    summary = {
        "model": "ChannelNetV6c_MSI",
        "config": CONFIG,
