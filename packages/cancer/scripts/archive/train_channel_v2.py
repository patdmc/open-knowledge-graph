#!/usr/bin/env python3
"""
Train ChannelNetV2 — enhanced model with tiers, cancer type, clinical covariates.
Checkpoints after every fold for resume capability.

Usage:
    python3 -u -m gnn.scripts.train_channel_v2
    python3 -u -m gnn.scripts.train_channel_v2 --resume  # resume from checkpoint
"""
...
SAVE_BASE = os.path.join(GNN_RESULTS, "channelnet_v2")
...
def run_fold(fold_idx, train_idx, val_idx, data_dict, config):
    train_ds = ChannelDatasetV2(data_dict, indices=train_idx)
...
def main():
    ...
    print("=" * 60)
    print("  CHANNELNET V2 TRAINING")
    print(f"  hidden={CONFIG['hidden_dim']}, layers={CONFIG['cross_channel_layers']}, "
          f"batch={CONFIG['batch_size']}")
    print(f"  Features: 6 channels + 3 tiers + cancer type + clinical")
    print("=" * 60)
