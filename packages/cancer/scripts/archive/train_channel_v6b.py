#!/usr/bin/env python3
"""
Train ChannelNetV6b — 8 channels, survival-filtered gene set.
Removes compensatory genes that dilute signal.

Usage:
    python3 -u -m gnn.scripts.train_channel_v6b
    python3 -u -m gnn.scripts.train_channel_v6b --resume
"""
...
from gnn.data.channel_dataset_v6b import (
    build_channel_features_v6b, ChannelDatasetV6b,
)
...
SAVE_BASE = os.path.join(GNN_RESULTS, "channelnet_v6b_filtered")
...
def run_fold(fold_idx, train_idx, val_idx, data_dict, config):
    train_ds = ChannelDatasetV6b(data_dict, indices=train_idx)
    val_ds = ChannelDatasetV6b(data_dict, indices=val_idx)
...
def main():
    ...
    print("=" * 65)
    print("  CHANNELNET V6b — 8 CHANNELS, SURVIVAL-FILTERED GENES")
    print(f"  hidden={CONFIG['hidden_dim']}, layers={CONFIG['cross_channel_layers']}, "
          f"batch={CONFIG['batch_size']}")
    print("=" * 65)

    data_dict = build_channel_features_v6b("msk_impact_50k")
    ...
    print(f"  vs ChannelNet V6 (8ch, 243g):    ~0.680")
    print(f"  vs Cox-Sage (RNA-seq):            0.63-0.72")
    print(f"{'='*65}")

    summary = {
        "model": "ChannelNetV6b_filtered",
        "config": CONFIG,
