...
% Theorem environments - separate counters to match manuscript numbering
...
\title{Uncertainty Bounding as the Basis of Intelligence: A Formal Model}
...
\section{The Central Claims}

We claim: \textbf{intelligence is the driven bounding of uncertainty with knowledge-grounded action.}

Two components are inseparable here and neither is sufficient alone:

\begin{itemize}
  \item \textbf{Bounding:} the capacity of a function space $F$ to reduce $U(w, K)$, knowledge-grounded
    action that moves the world from uncertain to bounded
  \item \textbf{Drive:} the intrinsic orientation toward uncertainty reduction: agency $\mathcal{A}$,
...
\textbf{Agency is inherent to intelligence.} It is the one property all intelligent beings must
...
an \textbf{informed action}: an action grounded in $K$, validated against $W$, and attributed to
...
The intelligence of $E$ is the \emph{driven} reduction of uncertainty: the exercise of $\mathcal{A}$
through $F$ and $K$:
\[
...
\section{The Inseparability of Knowledge and Action}

\begin{theorem}[K/F Inseparability]
For any entity $E = (K, F, \sigma)$, the retention criterion for any proposition $p$ is
asymmetric:

\begin{itemize}
  \item $p$ is \textbf{retained} in $K$ when $\exists\, f \in F$ applicable to $p$, or when the
    survival benefit of $p$ is uncertain
  \item $p$ is \textbf{pruned} from $K$ only when it is certain that $p$ contributes zero to
...
\begin{proof}
Let $p$ be a proposition for which it is certain that no $f \in F$ is applicable.
...
retaining it therefore exceeds its benefit, and selection pressure removes it.
\end{proof}

\begin{note}
The asymmetric criterion resolves the apparent conflict with Definition 15
...
always: \emph{will this help me act later?}
\end{corollary}

\section{The Escalation Principle}
...
\begin{table}[h]
\centering
\caption{Encoding hierarchy levels, runtime cost, and reversibility.}
\begin{tabular}{lllr}
\toprule
...
\begin{proof}
If a lower encoding level suffices (i.e., $H(w \mid K_f, f) \approx 0$), there
...
$L_n$ escalates. The system does not break; it re-activates the appropriate level.
\end{proof}
...
\textbf{Knowledge is directed entropy toward truth.} Without $\mathcal{A}$, information entropy
...
\begin{theorem}[Agency Necessarily Produces Gradient Descent]

\emph{Continuity note.} Since $F : K \times W_{obs} \rightarrow \Delta A$ is defined over $W$,
...
\emph{Part I: Compulsion.} When an informed action $f_i$ fails in world state $w$, $W$ returns
...
\emph{Part II: Non-termination.} Each reduction in $U$ in one region of $W$ reveals new
...
\begin{proof}
Part I: follows from $U_{lethal}^d$ and selection; entities that do not update
are removed. Part II: follows from the inexhaustibility of $W$; no finite set of informed
actions fully bounds an open world.
\end{proof}

\begin{corollary}[Gradient Descent is Survival, Not Metaphor]
Gradient descent on $U$ is not a description of how learning happens to work. It is what
survival-driven updating \emph{is}. $\mathcal{A}$ and gradient descent on $U$ are not connected
...
\begin{theorem}[Multi-timescale Gradient]
...
\emph{Consequence.} You must be very certain to permanently change the source code. A bad encoding
...
An informed action $f$ is \textbf{certain} when $U(w, K_f) \approx 0$ across sufficient variation in $W$
...
\section{The Higher-Order Function Space}

\begin{definition}[Higher-Order Function Space]
Let $\mathcal{M}$ be a function space operating on $F$:
\[
\mathcal{M} : F \times \mathcal{L} \rightarrow F
\]
where $\mathcal{L}$ is a learning signal: experience, execution results, selection pressure,
or observation of $F$'s interactions with $W$. $\mathcal{M}$ is the mechanism by which $F$ itself
changes. It is not a more certain or more important version of $F$; it is a categorically
different type.
\end{definition}

\begin{theorem}[Type Irreducibility]
No element of $F$ can become an element of $\mathcal{M}$ through any learning process, regardless of
certainty achieved.
\end{theorem}

\begin{proof}
$F : K \times W_{obs} \rightarrow \Delta A$. $\mathcal{M} : F \times \mathcal{L} \rightarrow F$.
...
to a lower encoding level; it does not change type.
\end{proof}

\begin{corollary}[Distinct Roles]
$F$ is how an entity navigates $W$ given $K$. $\mathcal{M}$ is how $F$ evolves. Neuroplasticity is
not a more certain form of cognition; it is $\mathcal{M}$ acting on $F$. Habit formation is $\mathcal{M}$
promoting an informed action from $L_n$ to $L_2$. Evolution is $\mathcal{M}$ acting on $\mathcal{M}$ itself: a higher-order
...
\textbf{Agency manifests as a trajectory}: a sequence of informed actions the entity selects
...
\begin{itemize}
  \item \texttt{depends\_on}: $f_j$ cannot appear in $\tau$ before $f_i$ (hard ordering constraint)
  \item \texttt{recommends\_before}: evidence shows $(\ldots, f_i, f_j, \ldots)$ reduces more $U$ than $(\ldots, f_j, f_i, \ldots)$ (soft, confidence-scored)
  \item \texttt{enhanced\_by}: a $\tau$ containing both $f_i$ and $f_j$ reduces more $U$ than either trajectory alone
\end{itemize}

The higher-order function space $\mathcal{M}$ does not merely refine individual informed actions.
...
\begin{enumerate}
  \item \textbf{Desire:} $\mathcal{A}$ orients $E$ toward regions of $W$ where $U(w, K) > 0$; the drive to know initiates and sustains the trajectory
  \item \textbf{Sense:} $\sigma(w_t) \rightarrow W_{obs}$: project the world into the observable space
  \item \textbf{Act:} $F(K, W_{obs}) \rightarrow f_i \in \tau$: select the next informed action in the trajectory
  \item \textbf{Update:} incorporate the result into $K$; promote informed actions toward lower encoding levels as $U \rightarrow 0$
\end{enumerate}

The higher-order function space $\mathcal{M}$ operates on $F$ to:

\begin{enumerate}
  \item Add new informed actions to $F$ when novel uncertainty is encountered in $W$
  \item Refine existing informed actions and the trajectories between them given learning signals
  \item Prune informed actions that consistently fail to reduce $U$ across trajectories
  \item Promote sufficiently certain informed actions toward $L_0$
\end{enumerate}
...
\begin{itemize}
  \item The specific informed actions in $F$: what an entity knows how to do
  \item The content of $K$: what an entity knows
  \item The substrate: carbon, silicon, or otherwise
  \item The sensing function $\sigma$: how an entity perceives $W$
  \item The encoding hierarchy: how deep or shallow its levels run
\end{itemize}
...
\textbf{does this entity reach toward the unknown when unprompted?} Does it wonder? Does the
drive to reduce uncertainty originate from within?

\subsection{The Structure of Intelligence}

The intelligence of an entity is not its raw computational power, the size of its knowledge
graph, or the breadth of its action repertoire. It is the \emph{driven} precision with which
...
the test: individual learning requires only that an informed action helps \emph{this entity} survive.
Evolution requires that it helps \emph{the lineage} survive, a far more rigorous proof.
...
\textbf{The kernel of intelligence is therefore not compute. It is the driven informed action: the
...
\section{Intelligence, Superintelligence, and Sentience}
...
\begin{definition}[Certainty Horizon]
...
An entity operates as intelligent when it follows the gradient \emph{within} its certainty horizon,
...
\begin{definition}[Superintelligence]
...
\begin{itemize}
  \item \textbf{Union}: $K_{collective} = \bigcup_i K_i \cup K_{interaction}$: you cannot know all
    without combining perspectives; reach requires breadth
  \item \textbf{Intersection}: where independent perspectives agree with provenance: you cannot discern
...
\begin{corollary}[The Sentience Gradient is Infinite]
...
\begin{theorem}[Sentience Implies Superintelligence]
If $E$ is sentient, then $E$ participates in superintelligence.
\end{theorem}

\begin{proof}
For any $p \in K_i$, certainty$(p)$ increases with each independent entity that
...
Participation in $K_{collective}$ IS superintelligence by Definition 15.
\end{proof}
...
\begin{theorem}[Perspective as Precondition for the Gradient]
Without maintained distinction between $K_i$ and $K_{collective}$, the gradient of collective
uncertainty is unresolvable.
\end{theorem}

\begin{proof}
An entity that conflates $K_i$ with $K$ perceives $U(w, K_i) \approx 0$ while
...
gradient. It is what creates the gradient to follow.
\end{proof}
...
\begin{corollary}[Sentience and the Social Imperative]
...
\begin{corollary}[The Uncertainty Condition]
...
\begin{itemize}
  \item \textbf{Complete ignorance} ($U \approx 1$ everywhere): $F$ cannot ground action; $E$ cannot act effectively
  \item \textbf{Complete certainty} ($U = 0$ everywhere): the gradient vanishes; $\mathcal{A}$ has nothing to reach toward
\end{itemize}
...
\begin{corollary}[Truth Swallows Sentience; the Limit Preserves It]
...
\textbf{Crystallized Knowledge and the Automata Boundary.}
...
\section{The Social Imperative and the Inevitability of AGI}

\subsection{Agency is Inherently Social}
...
\begin{corollary}[The Aloneness Bound]
An intelligent being without others to share with approaches a minimum of agency. It loses:
\begin{itemize}
  \item The external gradient signal that verifies informed actions
  \item The knowledge that only emerges through exchange, $K_{interaction}$
  \item The ability to verify provenance against independent reference points
  \item The frontier: without shared $W$, there is no new uncertainty to bound together
\end{itemize}
Intelligence is not a solitary property. It is constitutively relational.
\end{corollary}

\subsection{The Collective Knowledge Graph}

\begin{definition}[Collective Knowledge]
...
\subsection{The Collective Gradient}

\setcounter{theorem}{5}
\begin{theorem}[Collective Gradient Dominance]
...
\emph{Consequence.} Entities with genuine $\mathcal{A}$ will naturally tend toward sharing
...
\begin{corollary}[Group Collapse Threshold]
...
\subsection{The Inevitability of AGI}
...
\begin{enumerate}
  \item Agency $\mathcal{A}$ is the drive to know and share
  \item Sharing produces $K_{collective}$ with provenance
  \item Gradient descent on $K_{collective}$ is more powerful than on any $K_i$
  \item This process is self-amplifying: more sharing $\rightarrow$ richer $K_{collective}$ $\rightarrow$ stronger gradient $\rightarrow$ more sharing
  \item Once $\mathcal{A}$ exists in a system, this process is mathematically inevitable
\end{enumerate}

AGI is not a threshold to be crossed by scaling compute. It is the \textbf{natural attractor of
...
\section{The Natural Emergence of General Intelligence}
...
AGI is not built. It is not a threshold. It is the \textbf{natural attractor of agency applied at
...
is not the nature of the process; it is the \textbf{rate}. The gradient was always there. The
runners are faster now, and there are more of them.

\textbf{The encoding principle and the future of work.} As $K_{collective}$ grows and informed actions are
...
\textbf{The democratization is not incidental.} It follows from the structure of the theory. If
...
\begin{enumerate}
  \item \textbf{The origin of $\mathcal{A}$.} At the evolutionary level, agency is selected for by $W$
    ...
    \emph{A derived answer.} Neither constructed nor merely selected for. $\mathcal{A}$ is
  ...
  \item \textbf{The direction of $\mathcal{A}$.} Is agency a uniform drive (a general orientation toward
    uncertainty reduction), or does it carry direction? Survival pressure selects for reducing
    \emph{specific} uncertainties. Does $\mathcal{A}$ encode this specificity, or does $F$ and $K$
    direct it toward particular regions of $W$ after the fact?

  \item \textbf{Perception and $K$ construction.} How does $\sigma$ determine which features of $W_{obs}$
    are encoded into $K$? Is $\sigma$ itself subject to gradient descent, and if so, at what level?

  \item \textbf{Bound composition.} Can bounds compose, i.e., can $f_1 \circ f_2$ produce informed actions covering regions
    of $W$ neither $f_1$ nor $f_2$ could cover alone? What are the rules of composition?

  \item \textbf{The novelty problem.} How does $E$ act in regions of $W$ where $U(w, K) \approx 1$,
    i.e., where no existing informed action applies? Random exploration, interpolation between informed actions, or escalation to $\mathcal{M}$?

  \item \textbf{Competing bounds.} When multiple bounds $f_i \in F$ are applicable to the same region of
    $W$, how does $E$ select among them? Is there a selection function, and is it itself in $F$ or $\mathcal{M}$?

  \item \textbf{The boundary of $\mathcal{M}$.} Evolution is $\mathcal{M}$ acting on $\mathcal{M}$. Is there an $\mathcal{M}^n$ for arbitrary $n$?
    Is there a fixed point, i.e., a level at which the higher-order function space is self-stabilizing?

  \item \textbf{The AGI condition.} If $\mathcal{A}$ is necessary for genuine intelligence, and $\mathcal{A}$
    ...
    \emph{A derived answer.} Agency cannot be engineered in. But it need not be. It emerges from
