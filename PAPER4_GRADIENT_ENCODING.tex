...
descent on uncertainty $U$, with convergence guaranteed by a scalar
Lyapunov argument: $U \geq 0$ is bounded below, each update reduces
$\mathbb{E}[U]$ because novel encounters produce informative feedback,
and monotone convergence applies without dimensional decomposition. The gradient never terminates under positive novelty rate: as
long as the entity encounters states where $U > 0$, actions with
positive information gain exist. Second, the certainty threshold for encoding an action
...
\cite{Cover2006}. $G = 0$ if and only if $a$ provides no information
about $W$ beyond what $K$ already contains: the encounter is not
novel.
\end{definition}
...
(Definition~\ref{def:novelty}), the gradient is never globally zero:
...
=======
$\Delta_a = -\eta \cdot \delta$ revises $K$ in the direction that
reduces prediction error. $K$ is a set of propositions, not a
parameter vector, so $\nabla_K U$ is not defined in the classical
sense. The descent property is instead established directly: Step~1
below shows that each informative update strictly reduces
$\mathbb{E}[U]$, using the fact that conditioning on additional
evidence cannot increase expected uncertainty \cite{Cover2006}. This
is evaluation-driven state revision: the entity updates in a descent
direction on $U$ without requiring a differentiable parameterization
of $K$.
>>>>>>> 319f447daa46f8c1e949fb7929535d1aa6396025
...
=======
\emph{Step 1: Each $M$-update from a novel encounter strictly reduces
$\mathbb{E}[U]$.} When the entity encounters a state $w$ where
$U(w, K) > 0$ and executes action $a$, the encounter is informative:
$G(a, K) > 0$ because the feedback resolves uncertainty that $K$ did
not already contain. When $M$ integrates this feedback into $K$,
forming $K' = K \cup \{p_a\}$:
\[
\mathbb{E}[U(w, K') \mid K] \leq U(w, K) - G(a, K)
\]
The inequality is strict whenever $G(a, K) > 0$---that is, whenever
the encounter was novel. Novelty drives the descent: each novel
encounter produces an informative update, and each informative update
strictly reduces expected uncertainty.
>>>>>>> 319f447daa46f8c1e949fb7929535d1aa6396025
...
=======
$K$ from an informative encounter can only reduce $\mathbb{E}[U]$
(Step~1): integrating novel evidence never increases expected
uncertainty. Cross-grounding between
>>>>>>> 319f447daa46f8c1e949fb7929535d1aa6396025
...
The gradient is never globally zero. Descent converges locally
...
Not under the cost-benefit displacement criterion of the $K/A$
inseparability theorem. Every proposition $p \in K$ incurs a positive
maintenance cost $c_{\text{retain}}(p, K) > 0$: retrieval interference,
edge maintenance, and context consumption. Proposition $p$ is displaced
by $q$ only when
\[
...
\]
where $B$ is expected survival benefit and $\mathcal{T}$ is the
entity's task distribution. This is a \emph{net} criterion: it accounts
for the grounding lost when $p$ is removed, not just the grounding
gained by adding $q$. Displacement occurs only when the swap strictly
improves net survival value. Therefore $\mathbb{E}[U]$ does not
increase across displacements, and convergence is preserved---not
because $K$ grows, but because each displacement strictly improves the
net survival benefit of a fixed-size $K$.
>>>>>>> 319f447daa46f8c1e949fb7929535d1aa6396025
...
convergence argument is scalar; it operates on $U$ directly, with
no dependence on the dimensionality of the entity's internal
parameterization. Under positive novelty rate ($\nu(E) > 0$), the gradient never
terminates: novel states ensure actions with positive information
gain always exist.
