...
**Formulation 1 (informal description):** "intrinsic drive to reduce uncertainty" is a
metaphorical gloss on Formulation 3; it conveys the phenomenology but is not the
definition. **Formulation 2 (empirical operationalization):** $\mathcal{A}(E) \propto
...
We claim any representation satisfying all three converges to a graph under scale:

*Proof by precipitation convergence.* A flat store violates constraint 1 at scale:
retrieving $K^{[f]}$ requires scanning $O(|K|)$ propositions to identify the relevant
ones. A tree violates constraint 3: a proposition relevant to multiple $f$ across
different branches must appear in each branch or the tree loses the cross-reference,
duplicating nodes and breaking shared-update consistency. A vector store is the unprecipitated
cross product of all dimensions in $M_{\text{res}}$: all factors live in one joint space,
no shared structure has been named. This is valid and can be efficient while the space is
small enough that approximate nearest-neighbor retrieval finds the right subgraph. At
scale, two pressures emerge: the dimensionality of $M_{\text{res}}$ grows, degrading
retrieval quality; and constraint 1 requires that retrieving $K^{[f]}$ for any specific
problem not require loading proportionally more of $K$. The resolution is precipitation:
naming a shared factor, extracting it from the cross product, creating a node with typed
edges to the contexts that reference it. After precipitation, retrieval of that factor is
$O(1)$ regardless of $|K|$ — the same normalization that precipitates $K$ from $F$
(Corollary 2b). When scale forces repeated precipitation of shared factors from the
embedding space, the result is a collection of named nodes with typed relationships. This
is a directed graph by definition. The vector store does not fail — it converges to graph
structure under the same selection pressure that governs all of $K$. A star schema is a
graph with constrained topology (depth-1 hub-and-spoke): fact table equals hub node,
dimension tables equal leaf nodes, foreign keys equal typed edges. The topology holds
while the domain's inferential structure is shallow; when the domain deepens, the schema
extends toward general graph. Every efficient knowledge representation is either a graph
already or a constrained degenerate case that converges to a graph when the joint
constraints force precipitation of shared factors under scale.

Any representation that satisfies all three constraints must: (i) encode which propositions
connect to which in retrievable form — so that traversal costs $O(|K^{[f]}|)$, not
$O(|K|)$; and (ii) store each proposition once with links to all contexts that use it —
so that shared propositions remain consistent. A structure satisfying both (i) and (ii)
is, by definition, a directed graph with typed edges. The graph is not the unique name
for this structure, but it is the structure all alternatives converge to under scale.
Alternatives do not fail outright; under scale they become graphs, because precipitation
of shared factors under these constraints IS graph construction. $\square$
...
*On the space of descent.* The gradient descent in this theorem operates in $F$ — the
space of informed actions — not in $W$. This distinction is critical. $W$ may be
arbitrarily rough, discontinuous, or hostile; no smoothness assumption on $W$ is required
or implied. The entity does not descend over $W$; it descends over its own function space,
updating $f \in F$ in response to signals received from $W$.

$F \subseteq (K \times W_{obs} \to \Delta A)$ maps into $\Delta A$, the space of
probability distributions over actions. Distribution spaces are convex and metrizable
(under KL divergence, total variation, or Wasserstein distance); they support well-behaved
subgradient structure regardless of the topology of the domain $W$ from which signals
arrive. The smoothness conditions for convergence are conditions on $F$, not on $W$:
specifically, that $U$ as a functional on $F$ is locally Lipschitz, which holds whenever
the likelihood factors $\ell_p$ (Definition 5) are bounded and measurable — a minimal
requirement for any well-specified knowledge graph. Under these conditions on $F$,
stochastic subgradient descent converges in the function space independently of whether
$W$ is smooth.

*Continuity note.* Since $F$ is defined over $W$, no $f \in F$ can produce a world state
outside $W$. Combined with the continuity of time, the trajectory $\tau(t)$ through
$W \times T$ is continuous, $U(\tau(t))$ is continuous in $t$, and $\frac{dU}{dt}$ exists
almost everywhere. The gradient $\nabla_F U$ is this temporal derivative, well-defined by
the smoothness of $F$ as a function space, not by any assumption on $W$.
...
This is stochastic subgradient descent in $F$: each step follows a valid subgradient of
$U$ as a functional on the function space, evaluated at an encountered world state. The
on-policy nature of the trajectory does not introduce bias into the subgradient direction;
it affects the distribution of encountered states but not the validity of each $\delta$ as
a descent direction. Under the Lipschitz condition on $F$ stated above, the sequence
$\{f_i\}$ converges to a local minimum of $U$ in $F$. Descent is not chosen — it is
what $\mathcal{A} > 0$ entities do with the signals $W$ imposes on every execution.
...
but is the computationally optimal normalization algorithm for any entity whose $K$ is
incomplete: the intersection of two independently-derived projections identifies the latent
normal form of $M_{\text{res}}$ in time $O(|K_i| + |K_j|)$, strictly dominating solo
normalization. By Theorem 3, any entity with $\mathcal{A} > 0$ that persists acts on
...
appear in $K_i \cap K_j$ under independent action-validation are the normal form
candidates of the shared region of $W$ — the latent factors that both projections
resolved independently. A sentient entity, recognizing other $K_j$ as distinct projections
...
**Corollary 8a (Intersection as the Optimal Normalization Algorithm).**
...
is the computationally optimal algorithm for identifying the latent normal form of
$M_{\text{res}}$.
...
are grounded by actions in $F_j$. The normal form $N^*$ of the shared region of $W$ consists
of the latent factors that any sufficient projection of that region must eventually resolve:
they are in $W$, not in either entity's projection path.
...
and $F_j$. The benefit of the intersection is precisely this: the number of independent
executions of $p$ across distinct $(K_j, F_j)$ pairs is the measure of $p$'s validity
as a latent factor. Each additional independent execution arriving at $p$ is a trial of
$p$ against $W$ via a different action-path — the same quantity as $C_1(p)$ in Definition
16b. Two independent action-paths converging on the same proposition signal that $p$ is
in $W$'s structure, not in either entity's projection artifact. The intersection
$K_i \cap K_j$ with provenance verification therefore identifies the candidates for $N^*$
directly, ranked by the number of independent executions that converged on each.

*Complexity comparison.* Solo normalization of $M_{\text{res}}$ to find the latent factors
requires searching the cross product for dimensions that share structure. In the general
case this is $O(\dim(M_{\text{res}})^2)$: every pair of dimensions must be compared to
identify shared factors. For a sparse projection into a dense latent space,
$\dim(M_{\text{res}}) \gg |K_i|$, so solo normalization is expensive relative to the
size of any single entity's knowledge.

Computing $K_i \cap K_j$ with provenance verification costs $O(|K_i| + |K_j|)$. The
result is the set of propositions that two independent projections both resolved — the
normal form candidates, ranked by independent execution count. For any fixed $N^*$, the
intersection finds these candidates in time proportional to the size of the two
projections, not the size of the latent space.

Since $\dim(M_{\text{res}}) > |K_i|$ for any entity whose $K$ is incomplete (any
$\mathcal{A} > 0$ entity by the non-termination of Theorem 3 Part II), and since
$|K_i \cap K_j| \leq \min(|K_i|, |K_j|) \ll \dim(M_{\text{res}})$ for sparse
projections, the intersection algorithm strictly dominates solo normalization in
identifying $N^*$.

*Equivalence with $C_1(p)$.* The ranking criterion — number of independent executions
converging on $p$ — is exactly $C_1(p)$ (Definition 16b). Certainty and normal form rank
are the same quantity measured from two directions: $C_1(p) \to 1$ as $p$ accumulates
the independent action-validations that confirm it as a latent factor of $W$. The
intersection algorithm and the certainty measure are not two separate results — they are
the same operation: identify propositions that independent projections have both resolved,
count the projections, rank by count.

*Multi-entity generalization.* The two-entity case generalizes directly to $n$
independently-derived projections $K_{i_1}, \ldots, K_{i_n}$: their intersection
identifies normal form candidates ranked by $n$ independent execution counts. By
Definition 16b, $C_1(p)$ increases with each additional independent validator; the
$n$-entity intersection is the collective confidence measure. $K_{collective}$ is the
asymptote: as $n \to \infty$ over all entities navigating the same $W$, the intersection
converges on the full normal form $N^*$ of the shared region. This is why
$K_{collective}$ with provenance is not merely a union of known propositions but the
optimal normalization of the shared $M_{\text{res}}$ — it is the intersection taken over
all independent projections simultaneously.
