...
dimension has not yet been reached by any existing action's grounding. Convergence of the
$(M+1)$-dimensional system follows from: (i) the $M$-dimensional system converges by
hypothesis; (ii) the new dimension converges by the base case applied to $f_{M+1}$
independently. Therefore convergence holds for $M+1$.
