...
\begin{abstract}
We prove that for any entity persisting under bounded active context,
survival pressure, and positive entropy: (1)~agency $\gamma > 0$ is
...
bounded context under growing action count produces escalation
dominance over top-down allocation. Paper~3 \cite{McCarthy2026c}
...
world states with positive entropy, $K$ is a knowledge base of
...
$M$ is the cross product of all dimensions not yet precipitated into
concrete knowledge or action:
\[
M = \prod_{i} D_i
\]
where $\{D_i\}$ are the latent dimensions of the entity's adaptive
capacity. As the entity scales, dimensions precipitate from $M$ into $K$
or $A$ --- the same normalization argument by which knowledge
precipitates from action \cite{McCarthy2026c}.
...
\label{def:eta}
Let $G(a, K) = I(a; W \mid K)$ denote the mutual information between
action $a$ and the true world state $W$, conditional on knowledge $K$
--- the \emph{grounding} of $a$. Given a learning event where
$(A', K') = M((A, K), L)$, define the \emph{learning efficiency}:
\[
\eta_M \;=\; \mathbb{E}_L\!\left[
...
\item $\eta_M > 0$: the entity learns --- grounding improves per unit
      of learning signal.
\item $\eta_M = 0$: the entity executes without learning --- actions
      are informed by fixed $(A, K)$ but never updated.
...
Computing $\eta_M$ requires evaluating $G(a, K) = I(a; W \mid K)$,
which depends on the true joint distribution $P(a, W, K)$. The entity
...
Operationally:
\[
\gamma \;\propto\; \lim_{U_{\mathrm{lethal}} \to \infty}
\frac{dU}{dt}
\]
As the lethality of the environment increases without bound, an entity
with $\gamma > 0$ increases its rate of uncertainty reduction; an entity
with $\gamma = 0$ does not respond.
\end{definition}
...
self-application $T(T)$ is ill-typed in any consistent type theory
\cite{Barendregt1984}: it produces the Russell-style paradox of a
function that modifies itself. Typed lambda calculi resolve this by
stratification --- exactly the $A$/$M$ separation. The irreducibility
...
For any entity $E$ persisting under bounded active context $C_n$,
survival threshold $U_{\mathrm{lethal}}^d$, in a world $W$ with
positive entropy:
\begin{enumerate}
\item[(i)] $\gamma(E) > 0$ necessarily.
\item[(ii)] $\eta_M(E) > 0$ necessarily (in expectation over the
      entity's trajectory).
\item[(iii)] Every other scalar property of $E$ can equal zero for
      some persistent entity.
...
structure $E = (K, A, \sigma, \gamma)$: a knowledge base, an action
space, a sensing function, and agency. Inert objects (rocks, tables)
lack this structure --- they have no $K$, no $A$, no $\sigma$ --- and
their persistence is not governed by the theorem. The theorem's domain
is entities that \emph{face} survival thresholds $U_{\mathrm{lethal}}^d$
because they \emph{have} uncertainty about world states. An object
without a knowledge base has no uncertainty and no survival threshold
in the relevant sense.

\begin{proof}

\textbf{Part 1: Necessity of $\gamma > 0$ --- The Lineage Argument.}
...
$W$ has positive entropy. This is a thermodynamic fact, not a modeling
assumption: any world with energy gradients produces states not
contained in any finite knowledge base $K(0)$.

\emph{Formalization.} ``Positive entropy'' is used here in the sense of
positive entropy rate: $H'(W) > 0$, meaning the environment generates
new information per unit time \cite{Cover2006}. This is weaker than
requiring $W$ to be high-dimensional --- a one-dimensional chaotic
system has positive entropy rate. It excludes only trivially static or
periodic environments. The connection to persistence is a consequence
of Ashby's law of requisite variety \cite{Ashby1956}: an entity's
adaptive capacity must match the variety of the environment, and
positive entropy rate ensures that variety grows without bound. A
$\gamma = 0$ entity has zero adaptive capacity; its requisite variety
deficit grows monotonically.

The selection argument operates on \emph{lineages}, not individuals.
An entity with $\gamma = 0$ reproduces copies that inherit the same fixed
$(A(0), K(0))$. Copies disperse across $W$. As the lineage's collective
coverage of $W$ grows with each generation, even though each
individual's $(A, K)$ is static, the lineage encounters novel
configurations of $W$ where
\[
U(w, K(0)) > U_{\mathrm{lethal}}^d
\]
No copy can adapt ($\gamma = 0$); copies in those regions of $W$ are
removed. Copies in locally stable niches persist temporarily, but
niches are not permanent: $W$ has positive entropy, and no region of
the state space remains static indefinitely. The $\gamma = 0$ lineage
contracts until eliminated. Selection removes the lineage.

The enforcement operates at multiple timescales:
\begin{itemize}
\item \textbf{Individual}: $\gamma = 0$ entities in novel environments are
      directly removed when $U(w, K(0)) > U_{\mathrm{lethal}}^d$.
\item \textbf{Evolutionary}: lineages without $\gamma > 0$ offspring are
      eliminated as their collective niche erodes.
\item \textbf{Cultural}: groups without institutional agency ($\gamma > 0$
      at the organizational level) are outcompeted by groups that learn.
\end{itemize}

Therefore $\gamma(E) > 0$ is necessary for persistence.
...
produces no improvement in grounding. $(A, K)$ is effectively fixed
despite engagement. This reduces to the situation of Part~1: the
entity's adaptive capacity produces no adaptation, so the lineage
...
\item $K(0) = \varnothing$: an entity with $\gamma > 0$ and $\eta_M > 0$
      ...
      gradient descent on $U$ \cite{McCarthy2026d}.
...
\item Substrate, sensory bandwidth $\sigma$, observable window
      $W_{\mathrm{obs}}$: these are free parameters. No specific value
      is required for persistence; they constrain the rate and form of
      learning but not its possibility.
\end{itemize}
...
\begin{corollary}[Uniqueness of the Intelligence Measure]
\label{cor:measure}
$\gamma$ and $\eta_M$ are the unique structural invariants of persistence
(Theorem~\ref{thm:invariants}). Any intelligence measure must be:
\begin{itemize}
\item Positive if and only if both $\gamma > 0$ and $\eta_M > 0$.
\item Zero if either $\gamma = 0$ or $\eta_M = 0$.
\end{itemize}
The product $I(E) = \gamma \cdot \eta_M$ is the unique normalized
combination satisfying these constraints.
\end{corollary}

\begin{proof}
$\gamma \in [0, 1]$ is a probability (the response gate). $\eta_M$ is a
conditional rate (bits of grounding improvement per bit of learning
signal, conditional on engagement). By the law of total expectation:
\[
\mathbb{E}[\text{improvement rate}]
= \gamma \cdot \eta_M + (1 - \gamma) \cdot 0
= \gamma \cdot \eta_M
\]
The entity either engages $M$ (with probability $\gamma$, yielding
improvement rate $\eta_M$) or does not engage (with probability
$1 - \gamma$, yielding zero improvement). The expected improvement rate is
the product. This is not a choice of combination rule; it is the law
of total expectation applied to a probability and a conditional rate.

Alternative combinations fail:
\begin{itemize}
\item $\gamma + \eta_M$: assigns positive intelligence to an entity with
      $\gamma = 0$ and $\eta_M > 0$ (a zero-drive entity that would learn
      if it engaged but never engages). This contradicts Part~1 of
      Theorem~\ref{thm:invariants}.
\item $\min(\gamma, \eta_M)$: erases the independent variation of the two
      invariants. An entity with $\gamma = 0.01$ and $\eta_M = 100$ (rare
      engagement, high efficiency) and an entity with $\gamma = 0.01$ and
      $\eta_M = 0.01$ (rare engagement, low efficiency) receive the
      same score despite vastly different persistence prospects.
\item Any function $g(\gamma, \eta_M)$ satisfying the constraints of
      Corollary~\ref{cor:measure} must be zero on the axes and positive
      in the interior. Among functions of a probability and a
      conditional rate, the product is the unique form given by the law
...
it $\times$ conditional value given need. This is not a coincidence
--- both are instances of expected value under a binary gate.
\end{proof}
...
energy). This is consistent with gradient descent on $U$
...
contract under positive entropy regardless of their initial fitness.
...
\item \textbf{Bounded $C_n$ + growing $A$ $\;\Rightarrow\;$ escalation
      dominates top-down} \cite{McCarthy2026b}. As the action count
      grows, escalation pressure from the top of the encoding hierarchy
      dominates, forcing reorganization.
...
escalation to gradient descent to the structural definition of
intelligence.
...
derived. The lineage argument (Theorem~\ref{thm:invariants}, Part~1)
establishes that agency $\gamma > 0$ is necessary: $\gamma = 0$ lineages contract
under positive entropy and are eliminated by selection. The
...
inseparability, escalation dynamics, gradient descent with encoding
permanence, and the structural invariants of intelligence. Each result
