...
\begin{abstract}
We apply the bounded-context framework of Papers~1--5 to
...
to $C_k$-level optimization, where the action space available under
...
The bounded-context framework of Papers~1--5
\cite{McCarthy2026a, McCarthy2026b, McCarthy2026c, McCarthy2026d, McCarthy2026e}
...
Papers~1--5 derive their results from formal proofs; the present
...
We extend the entity framework of Papers~1--5 to multi-level
...
\lambda_j \cdot C_n^{(j)}
\]
where $\lambda_j \in (0, 1]$ is the coupling strength between levels
--- determined by the fidelity and bandwidth of intercellular
signaling. $\lambda_j = 1$ represents perfect coupling (all of
$E_j$'s context is available to $E_k$ via signaling);
...
Each $\lambda_j$ is thus not a single number but a \emph{vector} of
coupling channels. Chain severance can be partial: a cell may lose
...
\begin{theorem}[Escalation Chain Severance Produces Convergent
Malignancy]
\label{thm:cancer}
...
\textbf{Part III.} By escalation dominance \cite{McCarthy2026b}, with
...
\item \textbf{Proliferation}: cell division requires only local
   ...
   coordination needed.
\item \textbf{Metabolic autonomy}: glycolysis (the Warburg effect
   \cite{Warburg1956}) is the metabolic mode with the fewest
   tissue-level \emph{dependencies} --- it does not require reliable
   tissue-level oxygen delivery infrastructure. This does not imply
   mitochondrial shutdown (many cancer cells retain functional
   oxidative phosphorylation), but glycolytic capacity provides
   metabolic independence from tissue-level coordination.
\item \textbf{Motility}: movement in response to local nutrient
   gradients requires only cell-level sensing.
\item \textbf{Immune evasion}: downregulation of surface markers
   that signal to organism-level immune surveillance --- these markers
   are maintained by tissue-level signaling, and without that
   signaling, their expression decays.
\end{itemize}
...
dominance result \cite{McCarthy2026b}, this means all optimization
...
(Paper~5, \cite{McCarthy2026e}) guarantees continued engagement with
adaptive mechanisms. Its $\eta_M > 0$ guarantees learning from
...
reduces uncertainty about survival. Metastasis is exploration ---
the expansion of $T_{\mathrm{reachable}}$ into new spatial regions
--- driven by the same gradient that drives all learning
\cite{McCarthy2026d}.
...
The organism cannot distinguish between chain severance (cancer)
and chain mismatch (transplant). In both cases, the level $2 \to 3$
coupling signal is absent or aberrant, and the immune enforcement
mechanism responds identically: elimination.
...
\item \textbf{Convergent gene expression is constraint-determined,
   ...
   Specifically, recently evolved genes that serve cell-level
...
and $\eta_M > 0$ \cite{McCarthy2026e} guarantee that it will adapt.
...
\textbf{The seven-year puzzle.} The clinical observation that many
cancers can persist at low grade for years before rapid progression
is explained by the severance gradient: partial decoupling
($0 < \lambda_j < 1$) produces a cell that is partially constrained
by tissue-level signaling and partially operating under cell-level
optimization. This intermediate state can be stable while $\lambda_j$
remains above a threshold. Progression occurs when $\lambda_j$
crosses the threshold and cascading decoupling begins --- each loss
of coupling removes a constraint that was maintaining the remaining
coupling channels.
...
This paper has applied the bounded-context framework of Papers~1--5
...
rational optimizer --- and generates testable predictions
...
P.~D. McCarthy.
\newblock Gradient descent on uncertainty and encoding permanence from
  cost-of-failure.
\newblock Manuscript, 2026.

\bibitem{McCarthy2026e}
P.~D. McCarthy.
\newblock Agency and learning efficiency as the structural invariants of
  intelligence.
\newblock Manuscript, 2026.
