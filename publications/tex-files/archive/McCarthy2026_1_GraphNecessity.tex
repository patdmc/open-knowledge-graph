...
\title{Why Graph Structure Emerges:\\Consistency, Availability, and Bounded Context}
...
faces a fundamental tradeoff: it must be \emph{consistent}
(not lose or contradict what it knows) and \emph{available}
(retrieve and act on what it knows in sub-linear time). A flat
representation can provide one but not both---consistent storage
without structure requires $O(n)$ retrieval; available retrieval
without structure cannot scope invalidation cascades.
We show that a directed graph with typed edges is the cost-optimal
structure for providing both simultaneously. The argument proceeds
from two independent arguments, each corresponding to one side of
the tradeoff. The \emph{consistency argument} shows that the most efficient
space-creating operation that preserves information is factoring
--- extracting shared structure into named nodes with typed edges ---
and that factoring \emph{is} graph construction. The
\emph{availability argument} shows that any representation supporting
sub-linear dependency queries, contextual retrieval, and selective
removal with cascade necessarily encodes directed adjacency.
Together, these explain why graph structure independently emerges
across every domain that manages both consistency and availability
under bounded context.
We show that continuous-weight mechanisms (e.g., transformer
self-attention) increasingly store adjacency explicitly under scaling
pressure---sparse attention, KV caches, retrieval augmentation---reducing
the $O(n^2)$ cost of dynamic recomputation to $O(d)$, consistent with
the cost prediction. The result is substrate-independent, applying
to artificial neural networks, institutional knowledge systems, and
...
this series follows from their intersection.

The vise admits exactly two responses: minimize the need for context
or maximize context capacity. Strategy~1 keeps $|K|$ small (fungible
units, no hierarchy, partition tolerance through redundancy). Bacteria,
BitTorrent swarms, and transformer attention layers all follow this
pattern: flat, replaceable, and robust by sheer count. Strategy~2
structures $K$ into addressable hierarchy (typed edges, scoped
domains, sub-linear retrieval). At small scale, Strategy~1 does not merely
suffice---it \emph{outperforms}. When $|K| \ll C_n$, exhaustive search
is faster than indexed retrieval because the constant dominates the
exponent. The dangerous property of the inferior scaling class is not
that it fails but that it succeeds, conspicuously, until a crossover it
cannot see. The two cost curves intersect; past the intersection, only
Strategy~2 survives. Critically, multicellular life is not Strategy~2 replacing
Strategy~1. It is Strategy~1 \emph{organized by} Strategy~2: fungible
cells composed into a graph whose topology carries the knowledge.
Cancer, viewed from this framing, is the defection of cells from graph
back to swarm (Papers~5--6, \cite{McCarthy2026e, McCarthy2026f}).
...
Under scarcity---when environmental novelty exceeds what a single cell
can handle---the cells aggregate into a tubular network that solves
shortest-path problems through physical flow dynamics alone
\cite{Nakagaki2000}. The organism floods all paths simultaneously,
then prunes low-flow tubes until only the optimal routes remain:
parallel exploration followed by topological optimization. No neuron,
no central controller, no designer. The same cells, the same genome,
switching from swarm to graph when the pressure crosses the threshold.
Graph structure emerges from the constraint, not from design.
Large language models are undergoing the same transition now:
retrieval-augmented generation, tool use, and agent architectures are
graph structure aggregating around a flat substrate under scaling
pressure. The industry is slime-molding.

The intersection creates two demands that pull in opposite
directions. The system must be \emph{consistent}: when a proposition is
updated or invalidated, every dependent proposition must reflect the
change, or downstream inferences will contradict upstream evidence.
It must also be \emph{available}: when a problem requires a subset of
stored knowledge, that subset must be retrievable without examining
everything the system knows. In distributed systems, this tension is
formalized as the CAP theorem \cite{Brewer2000, Gilbert2002}: under
network partitions, a system cannot simultaneously guarantee
consistency and availability.

A knowledge system under bounded active context faces an analogous
constraint. Active context---the number of propositions simultaneously
available for inference---is the partition. Biological working memory holds
roughly $4 \pm 1$ items \cite{Cowan2001}. Transformer context windows,
though large, degrade in inference quality as they fill with content
irrelevant to the current problem \cite{Liu2024}. In each case,
there is a hard or effective bound $C_n$ on how many propositions can
be loaded at once. The system's total knowledge exceeds $C_n$, so at
any moment, most knowledge is partitioned away from active inference.

A flat representation (an unstructured list of propositions) can
serve one demand but not both. Consistency without structure requires
examining every proposition to propagate an update: $O(n)$ per
invalidation. Availability without structure provides no way to scope
which propositions depend on a changed one, so invalidation cascades
either propagate too broadly (wasting context) or too narrowly
(leaving stale dependents---inconsistency).

This paper shows that a directed graph with typed edges is the
cost-optimal structure for providing both consistency and availability
under bounded context, and explains why it independently emerges
across domains. We establish this from two independent
arguments, each corresponding to one side of the tradeoff:

\begin{enumerate}
\item \textbf{Consistency} (Proposition~\ref{thm:graph-info}).
   When a bounded system must make space for new propositions, the most efficient
   operation that creates space while preserving information is
   \emph{factoring}: extracting shared structure into a named node with
   typed edges. Factoring is graph construction. It preserves the
   posterior over world states exactly---the consistency guarantee.
   Discarding without adjacency information is blind; lossy compression
   without adjacency is blind discarding with extra steps.

\item \textbf{Availability} (Theorem~\ref{thm:graph-dynamics}).
   ...
   dependents) requires typed edges to scope correctly. These
   operations are the availability guarantee: maintaining knowledge
   under continuous evidence arrival without loading all of $K$ into
   context.
\end{enumerate}

The two arguments are independent: consistency alone favors graph
structure; availability alone favors graph structure. A system
requiring both---which is every system that persists under bounded
context with evidence arrival---converges on graph structure from
both directions.
...
connections). It shows that the \emph{information content} of any
cost-optimal representation converges on a directed graph, regardless of how that
graph is physically realized.

This has a direct consequence for the relationship between knowledge
and action. A highly connected graph creates consistency risk:
the more edges a node has, the larger the cascade when it is
updated. Separating knowledge from the action system that maintains
it---as current neural network architectures do, with weights
encoding knowledge and forward passes encoding action---creates two
representations that must be kept consistent with each other.
The consistency cost of that separation grows with graph density.
Paper~2 in this series \cite{McCarthy2026b} argues that under
bounded context, the separation cost exceeds its benefit: knowledge
and action converge toward a shared representation. The graph shown
to be cost-optimal here is not merely a data structure. It is the
architecture.

\subsection{Related work}

\textbf{The CAP theorem} \cite{Brewer2000, Gilbert2002} proves that a
distributed data store cannot simultaneously provide consistency
(every read returns the most recent write), availability (every
request receives a response), and partition tolerance (the system
operates despite message loss). The standard response is to choose
two: CP systems (e.g., ZooKeeper) sacrifice availability; AP systems
(e.g., Cassandra) sacrifice consistency; CQRS architectures maintain
separate read and write models to manage the tradeoff explicitly
\cite{Young2010}. The present paper shows that bounded active context
creates an analogous constraint for any knowledge system, and that
graph structure is the cost-optimal representation for managing both sides.

\textbf{Graph-structured knowledge representations} are widely used:
...
labeled graphs. In education research, Novak and Gowin~\cite{Novak1984}
showed that meaningful learning is the construction of concept maps---labeled
directed graphs where nodes are concepts and edges are named
relationships---and that knowledge which is not integrated into such a
structure remains inert. Novak~\cite{Novak1990} further demonstrated that
misconceptions are not wrong nodes but wrong edges: the learner has the
concepts but the relationships are incorrect. This is an independent
convergence on the same structural claim from the cognitive side.
These results demonstrate that graph structure is
\emph{effective}. The question of \emph{why} it independently emerges
across such diverse domains has received less formal attention. The
present paper provides an argument: graph structure is cost-optimal
because it is the most efficient representation for managing both
consistency and availability under bounded context.

\textbf{The information bottleneck} \cite{Tishby2000} establishes that
optimal representations compress input while preserving information
relevant to output. Our consistency argument
...
\textbf{Bounded rationality} \cite{Simon1955} establishes that cognitive
...
structure becomes the cost-optimal response.

\textbf{Codd's relational model} \cite{Codd1970} established that normalizing
...
Normalization is a consistency operation: it eliminates update anomalies
(where changing a fact in one place leaves stale copies elsewhere).
The consistency argument (Proposition~\ref{thm:graph-info})
derives the same operation from first principles: factoring
shared propositions into named nodes with typed edges is normalization,
and it is the \emph{unique} space-creating operation that preserves
information under bounded context.
...
% ------------------------------------------------------------------
\section{The Consistency Requirement: Graph from Information Preservation}
\label{sec:info}
% ------------------------------------------------------------------
...
% ------------------------------------------------------------------
\section{The Availability Requirement: Graph from Retention Dynamics}
\label{sec:dynamics}
% ------------------------------------------------------------------

We first derive the operations that any knowledge maintenance system
must support, then show that any representation supporting them
at sub-linear cost encodes directed adjacency.
...
% ------------------------------------------------------------------
\section{Continuous-Weight Mechanisms Converge Toward Graph Structure}
\label{sec:attention}
% ------------------------------------------------------------------
...
Proposition~\ref{thm:graph-info} and Theorem~\ref{thm:graph-dynamics} predict
is cost-optimal.
...
\textbf{Remark (Catastrophic forgetting as consistency failure).}
The consistency-availability framing explains a fundamental limitation
of continuous-weight knowledge stores. In a graph, every proposition
has an address and every dependency is a typed edge. When a proposition
is updated, the invalidation cascade follows the edges: the system
knows exactly what depends on the changed fact and can scope the
update. Typed edges are partition boundaries---they delineate which
knowledge is coupled to which.

A weight matrix has no partition boundaries. An update to any weight
potentially affects every output. There is no adjacency structure to
scope the cascade, because the matrix encodes no structural information
about which outputs depend on which inputs through which weights.
Consequently, learning a new fact can silently overwrite an existing
one, with no mechanism to detect or recover the loss. This is
\emph{catastrophic forgetting} \cite{McCloskey1989, Kirkpatrick2017}:
the consistency failure of an available system with no partition
structure.

Every proposed mitigation---elastic weight consolidation
\cite{Kirkpatrick2017}, experience replay \cite{Rolnick2019},
progressive networks \cite{Rusu2016}---is an attempt to bolt partition
boundaries onto a system that does not natively have them.
The graph structure shown to be cost-optimal in this paper provides those
boundaries natively: typed edges tell the system which knowledge
is coupled to which, so updates propagate correctly and nothing is
silently lost. This is not an engineering convenience; it is the
consistency guarantee that bounded-context systems require.

The cost comparison is stark. A graph provides $O(d)$ retrieval
(follow stored edges) and $O(d)$ scoped update cascade (propagate
along typed dependencies). A weight matrix provides $O(n^2)$
retrieval (full attention computation) and $O(n^2)$ unscoped update
cascade (gradient flows through all parameters). The weight matrix
is not merely inconsistent---it is $O(n^2/d)$ worse at both
retrieval and update than the graph it is dynamically recomputing.
It chose availability over consistency and got poor availability as
well, because recomputing structure from scratch at every step is
strictly more expensive than storing it.
...
The two results establish graph optimality from independent premises,
each corresponding to one side of the consistency-availability tradeoff.
Proposition~\ref{thm:graph-info} establishes the consistency
argument: the most efficient way to make space without losing information is
to factor shared structure into named nodes with typed edges.
Theorem~\ref{thm:graph-dynamics} establishes the availability
requirement: sub-linear dependency queries, contextual retrieval, and
scoped invalidation cascade all require directed adjacency.

In distributed systems, the analogous tension is formalized by the
CAP theorem \cite{Brewer2000}: no system can simultaneously guarantee
consistency, availability, and partition tolerance. Under bounded
active context, the partition is the context bound itself---most
knowledge is always partitioned away from inference. The graph
structure shown to be cost-optimal here is the representation that
most efficiently manages the resulting tradeoff: typed edges enable scoped consistency
(update anomalies propagate only along typed dependencies, not
globally), and adjacency enables sub-linear availability (retrieve
$K^{[f]}$ in $O(d)$ rather than $O(n)$).

This is not merely analogous to the engineering practice of
graph-structured data stores. Relational databases---directed graphs
over tables linked by foreign keys---remain the gold standard for
consistent, available data precisely because the relational model
enforces both sides of the tradeoff through normalization (consistency)
and indexing (availability). The present result argues that this is
not merely a design preference but a mathematical optimality: any system that
must be both consistent and available under bounded capacity converges
toward graph structure, regardless of substrate.
...
not a structural discovery. The objection: the theorems establish graph
optimality for an idealized system that no successful architecture
actually implements.
...
but that the cost-optimal structure is \emph{directed}, \emph{typed}, and
...
theorems identify \emph{which} structural properties are cost-optimal, not
...
Theorem~\ref{thm:graph-dynamics} shows that without stored directed
...
different implementations of the same structural optimum. Systems that
...
systems converge toward encoding adjacency to maintain inference quality under bounded
context.
...
We have shown that graph structure over propositions is the cost-optimal
representation for providing both consistency and availability under
bounded context, and have explained why it independently emerges across
every domain that faces this tradeoff. The consistency argument
(Proposition~\ref{thm:graph-info}) shows that factoring is the most efficient
information-preserving space-creating operation. The availability
argument (Theorem~\ref{thm:graph-dynamics}) shows that sub-linear
maintenance operations require directed adjacency. Together, they
explain why any system managing the consistency-availability
tradeoff under bounded context converges toward a directed graph
with typed edges, whether realized as explicit edges, learned sparse
attention, synaptic connectivity, or foreign keys in a relational
database.

Continuous-weight mechanisms such as transformer self-attention
increasingly store adjacency explicitly under scaling pressure,
reducing the $O(n^2)$ recomputation cost to $O(d)$. They pay $O(n^2)$ per inference step to
recompute the graph that a stored-adjacency system accesses in $O(d)$.
This is the availability cost of not storing the structure permanently.

\subsection*{The series from here}

This result establishes that graph structure is cost-optimal under
bounded context. The remaining papers in this series address the
consequences.
If knowledge is a graph, can it be separated from the action system
that maintains it? Paper~2 \cite{McCarthy2026b} argues that
separation creates a distributed consistency problem whose cost
exceeds the benefit under bounded context: knowledge and action converge
toward a shared representation, and the graph must be normalized with
typed edges that scope updates. This is the data store's consistency
property.
If the knowledge system is a normalized graph, what control
architecture manages it? Paper~3 \cite{McCarthy2026c} shows that
bounded context favors convergence toward escalation---handle
problems at the lowest sufficient level, escalate on failure---because
top-down supervision costs context that scales with action repertoire.
This is the service topology.
What measure of performance is appropriate for a system operating
under these constraints? Paper~4 \cite{McCarthy2026d} derives
$I = \gamma \cdot \eta_M$---the rate of adaptation to novelty---as
a natural measure of persistence under bounded context.
Together, these are not independent insights. They are consequences
of one constraint: bounded context forces the
consistency-availability tradeoff, and every architectural decision
follows from how that tradeoff is managed.

\subsection*{Computational consequence}

A proven structural result eliminates design space permanently.
A learning system operating over $n$ unstructured features must
evaluate $O(n^2)$ pairwise interactions to discover which ones
carry signal---rediscovering on every training run what the proof
already establishes. A system informed by the proven structure
evaluates only the $O(k^2)$ interactions the theory identifies as
meaningful, where $k \ll n$. The compression ratio $(n/k)^2$ is
the computational dividend of a proof. The proven structure does not
...
\bibitem{Rolnick2019}
D.~Rolnick, A.~Ahuja, J.~Schwarz, T.~P. Lillicrap, and G.~Wayne.
\newblock Experience replay for continual learning.
\newblock In \emph{Advances in Neural Information Processing Systems},
  volume~32, 2019.

\bibitem{Rusu2016}
A.~A. Rusu, N.~C. Rabinowitz, G.~Desjardins, H.~Sober, J.~Kirkpatrick,
...
\bibitem{Young2010}
G.~Young.
\newblock CQRS Documents.
\newblock \url{https://cqrs.files.wordpress.com/2010/11/cqrs_documents.pdf}, 2010.
...
P.~D. McCarthy.
\newblock Channel biology validates the encoding hierarchy: distributed
  coupling predicts survival where hub genes do not.
\newblock Working paper, 2026.
...
\bibitem{Novak1990}
J.~D. Novak.
\newblock Concept maps and {V}ee diagrams: Two metacognitive tools to
  facilitate meaningful learning.
\newblock \emph{Instructional Science}, 19(1):29--52, 1990.

\bibitem{Nakagaki2000}
T.~Nakagaki, H.~Yamada, and {\'A}.~T{\'o}th.
\newblock Intelligence: Maze-solving by an amoeboid organism.
\newblock \emph{Nature}, 407(6803):470, 2000.
