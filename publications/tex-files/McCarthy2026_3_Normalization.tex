...
matrices) cannot scope updates and therefore cannot maintain
consistency without global coordination at $O(n^2)$ cost.
...
a structural consistency tax that grows quadratically with capacity.
\end{abstract}
...
is a structural consequence of bounded context---and that the
structure which enforces it is the same structure that solves the
consistency problem in distributed systems.

\textbf{The consistency problem under bounded context.}
McCarthy \cite{McCarthy2026a} establishes that bounded active context
forces graph structure: typed edges over propositions, necessary for
...
edges create addressable partitions. The weight matrix cannot achieve
consistency at any bounded cost because there are no partitions to scope
the update. Techniques to impose partition structure on weight matrices
...
Paper~1~\cite{McCarthy2026a} proves that bounded context requires
graph structure for consistency and availability (the data store).
The present paper proves that bounded context forces
...
proves that bounded context forces escalation as the control
