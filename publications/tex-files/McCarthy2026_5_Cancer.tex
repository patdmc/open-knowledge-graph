...
The bounded context architecture paper \cite{McCarthy2026_BCA} proves that bounded
...
in \cite{McCarthy2026_BCA}.
McCarthy2026_BCA}. This paper can be read independently.
...
For each patient, we mapped somatic mutations to eight coupling
channels based on the organizational function of the mutated gene,
organized into four hierarchical tiers:
(1)~growth signaling (PI3K--AKT--mTOR, RAS--MAPK:
\emph{PIK3CA}, \emph{PTEN}, \emph{KRAS}, \emph{BRAF}, \emph{EGFR},
\emph{ERBB2}),
(2)~cell cycle checkpoint (\emph{TP53}, \emph{RB1}, \emph{CDKN2A},
\emph{CDK4/6}),
(3)~DNA damage response (DDR: \emph{BRCA1}, \emph{BRCA2},
\emph{ATM}, \emph{CHEK2}, mismatch repair genes),
(4)~tissue architecture (\emph{CDH1}, \emph{APC}, \emph{CTNNB1},
\emph{SMAD4}, \emph{NOTCH1}),
(5)~endocrine/hormone (\emph{ESR1}, \emph{GATA3}, \emph{FOXA1},
\emph{AR}),
(6)~immune surveillance (\emph{B2M}, \emph{HLA-A/B/C},
\emph{JAK1/2}),
(7)~chromatin remodeling (\emph{ARID1A}, \emph{SMARCA4},
\emph{KMT2C}, \emph{KMT2D}, \emph{SETD2}),
(8)~DNA methylation (\emph{DNMT3A}, \emph{TET2}, \emph{IDH1},
\emph{IDH2}).
Tiers pair complementary functions: cell-intrinsic (1--2),
tissue-level (3--4), organism-level (5--6), and meta-regulatory
(7--8). See \cite{McCarthy2026f} for the full tier analysis.
...
Cox regression in Table~\ref{tab:empirical} uses $n = 43{,}872$
...
count across three independent cohorts totaling 75{,}995 patients.
...
\midrule
MSK-IMPACT 2017 & Channel count (univariate) & 1.09 & $5 \times 10^{-13}$ & \\
\quad ($n=7{,}562$) & Log mutation count (univariate) & 1.16 & $2 \times 10^{-11}$ & \\
 & Channel count (multivariate) & 1.07 & 0.006 & Significant \\
 & Log mutation count (multivariate) & 1.05 & 0.32 & Not significant \\
\midrule
MetTropism & Channel count (univariate) & 1.05 & $1 \times 10^{-16}$ & \\
\quad ($n=24{,}561$) & Log mutation count (univariate) & 1.03 & 0.002 & \\
 & Channel count (multivariate) & 1.14 & $7 \times 10^{-29}$ & Significant \\
 & Log mutation count (multivariate) & 0.84 & $5 \times 10^{-16}$ & Protective \\
\midrule
MSK-IMPACT 50K & Channel count (univariate) & 1.04 & $6 \times 10^{-21}$ & \\
\quad ($n=43{,}872$) & Log mutation count (univariate) & 1.04 & $1 \times 10^{-7}$ & \\
 & Channel count (multivariate) & 1.09 & $4 \times 10^{-21}$ & Significant \\
 & Log mutation count (multivariate) & 0.92 & $3 \times 10^{-8}$ & Protective \\
\bottomrule
\end{tabular*}
\end{table}
...
\midrule
MSK-IMPACT 2017 ($n=7{,}574$) & 32.3\% & 26.7\% &
  $4.7 \times 10^{-5}$ \\
MetTropism ($n=25{,}659$) & 42.6\% & 37.1\% &
  $8.8 \times 10^{-19}$ \\
MSK-IMPACT 50K ($n=44{,}283$) & 42.1\% & 38.9\% &
  $6.0 \times 10^{-13}$ \\
\bottomrule
\end{tabular}
\end{table}
...
mutations survive longer. At 2 channels: mortality drops from 43.8\%
to 41.3\% ($p = 6.8 \times 10^{-4}$). At 3 channels: 45.8\% to
42.8\% ($p = 4.3 \times 10^{-4}$). MetTropism replicates: at
3 channels, 48.4\% vs.\ 43.7\% ($p = 1.8 \times 10^{-3}$).
...
severed (Figure~\ref{fig:km}). In the 50K cohort: 26.9\% (0~channels), 37.3\% (1),
42.6\% (2), 44.6\% (3), 43.9\% (4). Patients with 6--8 channels
show lower mortality (33.2\%, 23.4\%, 21.1\%), consistent with the
...
\textbf{Sensitivity to channel granularity.} The eight-channel mapping
...
\begin{table}[ht]
\centering
\caption{Sensitivity analysis (MSK-IMPACT 50K, $n = 43{,}872$):
...
significant from 2 through 8 channels, peaks at 3--4, and collapses
...
\midrule
2 & 1.11 & $1 \times 10^{-17}$ & 0.95 & $3 \times 10^{-4}$ \\
3 & 1.20 & $1 \times 10^{-69}$ & 0.85 & $1 \times 10^{-31}$ \\
4 & 1.21 & $3 \times 10^{-75}$ & 0.88 & $9 \times 10^{-27}$ \\
5 & 1.11 & $3 \times 10^{-29}$ & 0.90 & $2 \times 10^{-12}$ \\
6 & 1.09 & $4 \times 10^{-21}$ & 0.92 & $3 \times 10^{-8}$ \\
7 & 1.05 & $2 \times 10^{-9}$ & 0.96 & $9 \times 10^{-3}$ \\
...
The signal peaks at 3--4 channels ($p = 3 \times 10^{-75}$) and is
...
(which of the eight channels is mutated) is significant in 22 of 29
...
eight-channel Cox model and the degree-only $p$-value for the same
cohort. 122 genes mapped to 8 channels across 468 panel genes.
...
The eight coupling channels are not severed at random
(Figure~\ref{fig:hierarchy}). Among patients
with exactly one channel severed ($n = 10{,}807$), 65.7\% have
PI3K/Growth Signaling or Cell Cycle: the cell-intrinsic channels
that govern proliferation. Chromatin Remodeling appears in 12.9\%,
consistent with its meta-regulatory role.
Endocrine and Immune account for fewer
than 5\%. Among patients with two or more channels severed
($n = 31{,}156$), 94.5\% have at least one cell-intrinsic channel.

The progression ladder is:
\begin{itemize}
\item \textbf{1~channel}: PI3K/Growth (35.6\%) or Cell Cycle
  (30.0\%) dominate. Chromatin Remodeling: 12.9\%. Immune: 0.7\%.
\item \textbf{2~channels}: PI3K/Growth rises to 59.9\%, Cell Cycle
  to 56.8\%. Chromatin Remodeling enters at 29.1\%.
  Tissue Architecture at 25.4\%.
\item \textbf{3~channels}: PI3K/Growth at 78.4\%, Cell Cycle 69.6\%,
  Chromatin Remodeling 52.1\%, Tissue Architecture 50.0\%.
  Endocrine (9.9\%) and Immune (4.1\%) remain rare.
\item \textbf{4~channels}: PI3K/Growth at 87.7\%, Chromatin
  Remodeling 75.7\%. Endocrine and Immune are the last additions
  (16.4\%, 9.9\%).
\end{itemize}
...
The ordering is PI3K/Growth $\to$ Cell Cycle $\to$ Chromatin
Remodeling $\to$ Tissue Architecture $\to$ DDR $\to$ Endocrine
$\to$ Immune. Growth escape is the gateway; immune evasion is the
last system to fall. Chromatin Remodeling appearing third is
consistent with its Tier~3 (meta-regulatory) role: disrupting
the machinery that controls gene expression enables subsequent
channel breaks. Single-channel patients reflect
...
The 5.5\% of multi-channel patients who lack a cell-intrinsic
gateway are enriched for prostate,
breast, and mesothelioma: cancers entering through
organism-level channels. Their lower mortality (37.8\% vs.\ 42.3\%)
...
\cite{McCarthy2026_BCA}. Coupling channels are the independent
...
\cite{McCarthy2026_BCA}. What remains: proliferation,
...
Five classes of prediction follow from the projection framework:
expanded eligibility, combination futility, combination timing,
untested targets, and immunotherapy sequencing.
...
\subsection{Immunotherapy candidacy after channel-level therapy}

Channel-level therapies---hormone therapy, PARP inhibitors---suppress
the channel they target but do not eliminate the tumor. Cancer cells
under sustained suppression continue accumulating mutations, but with
the primary escape route blocked, the majority are passengers: they
confer no selective advantage but each produces neoantigens visible
to the immune system.

The framework predicts that effective channel-level therapy
inadvertently marks residual cancer cells for immune recognition.
The longer the therapy holds, the more passenger mutations
accumulate, and the richer the neoantigen profile becomes. This
produces two testable predictions:

\textbf{Prediction 1: Passenger accumulation under therapy.}
Patients responding to channel-level therapy (adjuvant hormone
therapy, PARP inhibitors) will show increasing passenger mutation
burden over time, detectable via serial liquid biopsy (ctDNA).
These passengers are not drivers of resistance---they are failed
escape attempts. Each represents a neoantigen the immune system
can target.

\textbf{Prediction 2: Long-term tamoxifen patients who progress are
immunotherapy candidates.} ER+/PR+ breast cancer patients who
respond to adjuvant tamoxifen for five or more years and then
progress will respond to immune checkpoint inhibitors at rates
exceeding current expectations for hormone-receptor-positive
disease. The selection criterion is not receptor status or TMB at
diagnosis; it is duration of prior adjuvant response. Years of
suppressed-but-present disease produce a clean neoantigen
profile---mostly passengers, few drivers---that is ideal for
immune targeting.

ER+ breast cancer is considered non-immunogenic based on low PD-L1
expression and low TMB at diagnosis. The framework predicts this
assessment is incorrect for the specific subpopulation of long-term
adjuvant responders at first progression, whose neoantigen profile
at the time of progression differs fundamentally from their profile
at diagnosis.

Triple-negative breast cancer responds to checkpoint inhibitors
because genomic instability produces abundant neoantigens rapidly.
The long-term adjuvant hormone patient reaches a comparable
neoantigen state through a different route: slow accumulation under
therapeutic suppression. The marks are cleaner (fewer co-occurring
driver mutations) but narrower (fewer total neoantigens). Whether
this cleaner profile produces better or worse checkpoint response
is an empirical question the framework identifies but cannot
resolve without data.

\textbf{Testable claim:} Among ER+ breast cancer patients treated
with checkpoint inhibitors at progression, objective response rate
will correlate with duration of prior adjuvant endocrine therapy,
not with TMB at initial diagnosis.

\textbf{Prediction 3: TMB conditioned on channel count predicts
immunotherapy response.} TMB is a weak predictor of overall
survival (Section~2) because it conflates two signals: channel
escalations (driver mutations that breach new defense layers) and
passengers (mutations that breach nothing). For survival, only
escalations matter. For immunotherapy response, passengers matter
---each is a neoantigen the immune system can target.

The framework predicts that TMB conditioned on channel count will
outperform TMB alone as a predictor of checkpoint inhibitor
response. A patient with high TMB and few disrupted channels
carries mostly passengers---abundant neoantigens, few escape
routes, ideal for immune targeting. A patient with the same TMB
spread across many channels carries mostly escalations---each
mutation opened a real escape route, leaving fewer marks and more
resistance.

Triple-negative breast cancer illustrates this directly. TNBC
tumors that respond to checkpoint inhibitors should have high TMB
relative to their channel count---cancers that failed many escape
attempts and left fingerprints. TNBC tumors that resist checkpoint
should have low TMB relative to channel count---cancers that found
escape routes efficiently, leaving few marks for immune to target.

\textbf{Testable claim:} Among checkpoint-treated patients in any
cancer type, the ratio of mutation count to channel count will
predict objective response rate. High ratio (many passengers per
channel) predicts response; low ratio (few passengers per channel)
predicts resistance. This can be tested immediately using existing
clinical trial datasets that include both genomic sequencing and
...
appear in \cite{McCarthy2026_BCA}.
McCarthy2026_BCA}.
...
pathway membership alone. The full mapping (122 genes) and analysis
