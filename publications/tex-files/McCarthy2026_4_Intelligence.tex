...
A cancer cell that has severed its coupling channels operates under
cell-level bounded context alone. Across 73,593 patients, these cells
converge on the same phenotypic endpoints, uncontrolled proliferation,
immune evasion, metabolic reprogramming, regardless of tissue of origin
or mutational path \cite{McCarthy2026e}. Convergence under constraint is
evaluation-driven descent: the cell optimizes against a reduced
objective because its context no longer includes the organism.
The present paper identifies what this descent, and the intelligence
measure it implies, select for under bounded context.
...
Remark~1). Paper~2 \cite{McCarthy2026b} established that bounded context forces the
...
\item \textbf{Architectural efficiency}: the asymptotic cost of
   ...
   local degree. An entity with collapsed parameters pays $O(n^2)$
   per update, where $n$ is the total parameter count exposed to the
   update.
\end{enumerate}
...
An entity with $O(n^2)$ architecture and large $C_n$ will hit its
