...
\title{Life Under Bounded Context: Descent, Encoding Permanence,\\and What Persists}
...
minimal structural invariants of persistence: the free-context
exhaustion argument shows that $\gamma = 0$ entities are eliminated
under positive novelty rate; without $\eta_M > 0$, active context
fills without relief and inference degrades to failure. These are
necessary conditions for persistence, not sufficient ones---physical
integrity, energy supply, and substrate constraints are also required
but are not information-theoretic invariants. Within the
information-theoretic frame,
$I(E) = \gamma \cdot \eta_M$---the rate of adaptation to novelty---is
a natural measure of intelligence, identified by what positive novelty
rate selects for.
\end{abstract}
...
Any entity that persists under finite resources and positive novelty
must update its model of the world or be eliminated. This is true of
organisms, organizations, software systems, and individual cells.
The present paper identifies what this requirement selects for:
the minimal structural invariants of persistence under bounded context,
and a natural measure of intelligence derived from them.

In simplest terms: life is a bounded context optimization problem. Any entity that persists under finite resources and positive novelty must update its model of the world or be eliminated. The present paper identifies what that requirement selects for.

Paper~1 \cite{McCarthy2026a} established that any entity accumulating
propositions under bounded context converges on a directed
graph with typed edges as the cost-optimal representation, where ``necessary'' means selected for under
competition, not logically impossible to avoid (see \cite{McCarthy2026a},
Remark~1). Paper~2 \cite{McCarthy2026b} established that bounded context favors the
normalization of shared propositions from action into a separated $K$,
with an asymmetric retention criterion. Paper~3 \cite{McCarthy2026c} established that
bounded context under growing action count favors convergence toward
escalation.
...
learning efficiency $\eta_M > 0$ are the minimal structural invariants
of persistence, and Corollary~\ref{cor:measure} derives
$I(E) = \gamma \cdot \eta_M$ as the natural intelligence measure.
...
via selection and exhaustion arguments, the minimal structural
...
\end{itemize}
Escalation \cite{McCarthy2026c} pushes proven actions
...
Computing the exact value of $\eta_M$ requires evaluating
$G(a, K) = I(p_a; W \mid K)$, which depends on the true joint
distribution $P(p_a, W, K)$. The entity cannot compute its own
$\eta_M$; it would need the very information it is trying to learn.

However, $\eta_M$ need not be measured exactly to be compared across
entities. The ceiling on $\eta_M$ is determined by two independent
constraints:

\begin{enumerate}
\item \textbf{Functional context size} $C_n$: the substrate-determined
   bound on active context. This is the size of the pipe. It cannot be
   changed by the entity.

\item \textbf{Architectural efficiency}: the asymptotic cost of
   integrating new knowledge into $(A, K)$. This is how efficiently
   the entity packs the pipe. An entity with separated $K$, $A$, $M$
   and stored adjacency pays $O(d)$ per update, where $d$ is the
   local degree. An entity with collapsed parameters lacks partition
   boundaries for scoping updates, manifesting empirically as
   catastrophic forgetting \cite{McCloskey1989,Kirkpatrick2017}.
\end{enumerate}

As $|K|$ grows, a suboptimal architecture fills $C_n$ faster. The
rate at which performance degrades under increasing knowledge load is
observable: it is the empirical signature of the architectural class.
An entity without partition boundaries and large $C_n$ will hit its
context boundary before an entity with $O(d)$ architecture and small
$C_n$, once knowledge load is sufficient. This is the crossover of
Paper~3 \cite{McCarthy2026c} expressed as a measurement principle.

The function signature dominates over the coefficients. Within an
architectural class, the exact value of $\eta_M$ depends on
coefficients (quality of evidence integration, accuracy of the model).
Across architectural classes, the asymptotic cost determines the
ceiling. Comparing $\eta_M$ across entities is therefore a Big-$O$
problem: identify the architectural class (collapsed vs.\ separated
vs.\ recursive), and the class determines the bound. The class is
directly observable from the entity's structure without requiring
access to the true joint distribution.
\end{remark}
...
(see the model-vs-reality remark in \cite{McCarthy2026b}, Lemma~1).
...
the curation regime established in \cite{McCarthy2026b}, $|K|$ is
...
Under the cost-benefit displacement criterion of \cite{McCarthy2026b},
...
\cite{McCarthy2026c}, errors partition into \emph{handled errors},
...
\textbf{The relational database clarification.}
$K/A$ inseparability (Paper~2, \cite{McCarthy2026b}) does not imply
that $M$ is inseparable from $(A, K)$. In a relational database, data
(rows) and queries (SELECT, INSERT) are inseparable: queries are
defined over tables, tables are designed for queries. Neither makes
sense without the other. But DDL (ALTER TABLE, CREATE INDEX) is
obviously separate from DML. Schema migration operates on the
structure of tables and queries to produce new tables and queries.
No one confuses ALTER TABLE with SELECT. The type signatures are
different, the rate of change is different, and the database enforces
the separation. $K/A$ inseparability says data and operations over
data co-define each other. $A/M$ stratification says operations that
\emph{change} the data-and-operations are a categorically different
function. Inseparability is not transitive.
...
This is exactly the normalization argument of \cite{McCarthy2026b}
...
% ------------------------------------------------------------------
\section{What Persists Under Positive Novelty}
\label{sec:necessity}
% ------------------------------------------------------------------
...
% ------------------------------------------------------------------
\section{The Intelligence Measure}
\label{sec:measure}
% ------------------------------------------------------------------
...
differences: Chollet's skill-acquisition efficiency is structurally the closest prior work to $\eta_M$. The key difference: Chollet treats prior knowledge $K$ as a confound to be
...
\item \textbf{Bounded $C_n$ + information preservation $\;\Rightarrow\;$
      ...
      directed graph with typed edges. (The data store.)

\item \textbf{Bounded context $\;\Rightarrow\;$ normalization of shared
      propositions into $K$, with asymmetric retention}
      \cite{McCarthy2026b}. Overlapping action policies under bounded
      ...
      value. (The data store's consistency properties.)

\item \textbf{Bounded $C_n$ + growing $A$ $\;\Rightarrow\;$ convergence
      to escalation} \cite{McCarthy2026c}. Any viable control
      architecture converges toward escalation properties as concurrent
      demands grow relative to $C_n$. (The service topology.)
...
\item \textbf{$\gamma > 0$ + feedback $\;\Rightarrow\;$ descent on $U$
      ...
      arguments establish the two invariants; their product is a natural
...
\item \textbf{The $M^{(n)}$ hierarchy closes by recursion.}
      The relational database analogy clarifies the stratification.
      $K$ is data (rows in tables). $A$ is DML: operations on data
      (SELECT, INSERT, UPDATE). $M$ is DDL: operations on schema
      (ALTER TABLE, CREATE INDEX). $M' = S$ is system architecture:
      operations on DDL (choice of engine, sharding topology,
      replication strategy). Each level operates on the level below
      with a different type signature and a different rate of change,
      corresponding to the encoding hierarchy with $\theta_i$
      increasing at each level.

      The hierarchy is not open upward. $S$ decomposes into its own
      $(K_S, A_S, M_S)$: the architect needs knowledge about
      architectures, actions to implement them, and the capacity to
      learn which architectures work. When $M_S$ operates on $S$
      itself, the loop closes: the system can redesign its own
      architecture. This recursion is the natural termination point.

      After closure, further refinement of $M$ is edge identification
      within the existing graph, not level creation. The normalization
      argument of Paper~2 \cite{McCarthy2026b} applies: shared
      propositions precipitate, new edges form between existing nodes,
      the graph gets richer. The cycle does not change. Growth after
      closure is knowledge accumulation within a complete architecture.

      The open question reduces to: how does $M$ decompose across
      dimensions of $W$ before and after closure? The decomposition is
      edge identification (which typed relationships connect which
      components of $M$), not architectural (how many levels exist).
      An entity whose $S$ has $\gamma_S > 0$---whose architecture
      itself learns---operates at the recursive level. Whether this
      constitutes what is colloquially called ``superintelligence''
      is an empirical question about the consequences of the recursion,
      not a separate theoretical construct.
...
The normalization result \cite{McCarthy2026b} establishes that
...
retrieval recomputes relevance dynamically, provenance is absent,
and the knowledge is neither inspectable nor correctable. The prediction: open knowledge
...
the compute cost. Structured retrieval with attribution is
...
\cite{McCarthy2026c} rather than monolithic inference,
...
escalation chains.}
The escalation hierarchy \cite{McCarthy2026c} predicts that
...
one or more lower-level error handlers. Cellular errors that should
...
Intelligence is the rate of adaptation to novelty. This is identified by elimination: positive novelty rate requires $\gamma > 0$
and $\eta_M > 0$; their product is the unconditional rate. The identification is a selection argument, not a logical derivation: entities lacking these properties are eliminated, not logically impossible.
...
Theorem~\ref{thm:invariants} establishes that $\gamma > 0$ and
$\eta_M > 0$ are the minimal structural invariants of persistence under
...
entities. The product $I(E) = \gamma \cdot \eta_M$ is the natural measure
...
Any monotone transform of $I(E)$ preserves the ordering over entities;
we claim naturality (the measure falls out of the derivation without
arbitrary choices), not uniqueness up to transformation.

The four-paper chain identifies, from the single constraint of bounded
active context, the architecture that persists: graph structure,
...
P.~D. McCarthy.
\newblock Why graph structure emerges: consistency, availability, and bounded context.
