...
The graph structure proven necessary in this paper provides those
...
requirement: the only way to make space without losing information is
...
structure proven necessary here is the minimum representation that
manages the resulting tradeoff: typed edges enable scoped consistency
...
and indexing (availability). The present result proves that this is
not a design preference but a mathematical necessity: any system that
must be both consistent and available under bounded capacity must
maintain graph structure, regardless of substrate.
...
choice but the minimum representation that provides both consistency
and availability under bounded context. The consistency argument
(Proposition~\ref{thm:graph-info}) shows that factoring is the unique
...
establish that any system managing the consistency-availability
tradeoff under bounded context necessarily maintains a directed graph
...
implement graph structure dynamically rather than constituting
alternatives to it. They pay $O(n^2)$ per inference step to
...
This result establishes that graph structure is necessary.
The remaining papers in this series address the consequences.
If knowledge is a graph, can it be separated from the action system
that maintains it? Paper~2 \cite{McCarthy2026b} proves that
separation creates a distributed consistency problem whose cost
exceeds the benefit under bounded context: knowledge and action must
share a single representation, and the graph must be normalized with
...
architecture manages it? Paper~3 \cite{McCarthy2026c} proves that
bounded context forces convergence toward escalation---handle
...
the unique structural invariant of persistence under bounded context.
