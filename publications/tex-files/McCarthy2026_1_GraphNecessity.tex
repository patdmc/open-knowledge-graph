...
\title{Graph Structure Is Necessary for\\Managing Consistency and Availability Under Bounded Context}
...
We prove that a directed graph with typed edges is the minimum
structure that provides both simultaneously. The proof proceeds
from two independent arguments, each corresponding to one side of
the tradeoff. The \emph{consistency argument} shows that the only
space-creating operation that does not lose information is factoring
...
Together, these establish that graph structure is not a design
preference but the unique representation that manages both
...
alternative to it. The result is substrate-independent, applying
...
This paper proves that a directed graph with typed edges is the
minimum structure that provides both consistency and availability
under bounded context. We establish this from two independent
...
The two arguments are independent: consistency alone forces graph
structure; availability alone forces graph structure. A system
requiring both---which is every system that persists under bounded
context with evidence arrival---is doubly committed.
...
Paper~2 in this series \cite{McCarthy2026b} proves that under
bounded context, the separation cost exceeds its benefit: knowledge
and action must share a single representation. The graph proven
necessary here is not merely a data structure. It is the
architecture.
...
graph structure is the minimum representation that manages both sides.
...
\emph{effective}. The question of whether it is \emph{necessary} has
received less formal attention. The present paper provides the missing
argument: graph structure is necessary because it is the minimum
representation that provides both consistency and availability under
bounded context.
