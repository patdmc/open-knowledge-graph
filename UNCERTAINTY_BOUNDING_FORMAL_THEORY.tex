...
We claim any representation satisfying all three converges to a graph
under scale:

\emph{Proof by precipitation convergence.} A flat store violates
constraint 1 at scale: retrieving \(K^{[f]}\) requires scanning
\(O(|K|)\) propositions to identify the relevant ones. A tree violates
constraint 3: a proposition relevant to multiple \(f\) across different
branches must appear in each branch or the tree loses the
cross-reference, duplicating nodes and breaking shared-update
consistency. A vector store is the unprecipitated cross product of all
dimensions in \(M_{\text{res}}\): all factors live in one joint space,
no shared structure has been named. This is valid and can be efficient
while the space is small enough that approximate nearest-neighbor
retrieval finds the right subgraph. At scale, two pressures emerge: the
dimensionality of \(M_{\text{res}}\) grows, degrading retrieval quality;
and constraint 1 requires that retrieving \(K^{[f]}\) for any specific
problem not require loading proportionally more of \(K\). The resolution
is precipitation: naming a shared factor, extracting it from the cross
product, creating a node with typed edges to the contexts that reference
it. After precipitation, retrieval of that factor is \(O(1)\) regardless
of \(|K|\) --- the same normalization that precipitates \(K\) from \(F\)
(Corollary 2b). When scale forces repeated precipitation of shared
factors from the embedding space, the result is a collection of named
nodes with typed relationships. This is a directed graph by definition.
The vector store does not fail --- it converges to graph structure under
the same selection pressure that governs all of \(K\). A star schema is
a graph with constrained topology (depth-1 hub-and-spoke): fact table
equals hub node, dimension tables equal leaf nodes, foreign keys equal
typed edges. The topology holds while the domain's inferential structure
is shallow; when the domain deepens, the schema extends toward general
graph. Every efficient knowledge representation is either a graph
already or a constrained degenerate case that converges to a graph when
the joint constraints force precipitation of shared factors under scale.

Any representation that satisfies all three constraints must: (i) encode
which propositions connect to which in retrievable form --- so that
traversal costs \(O(|K^{[f]}|)\), not \(O(|K|)\); and (ii) store each
proposition once with links to all contexts that use it --- so that
shared propositions remain consistent. A structure satisfying both (i)
and (ii) is, by definition, a directed graph with typed edges. The graph
is not the unique name for this structure, but it is the structure all
alternatives converge to under scale. Alternatives do not fail outright;
under scale they become graphs, because precipitation of shared factors
under these constraints IS graph construction. \(\square\)
...
minimum of \(U\) in \(F\). Descent is not chosen --- it is what
\(\mathcal{A} > 0\) entities do with the signals \(W\) imposes on every
execution.
...
gradient-preferred but is the computationally optimal normalization
algorithm for any entity whose \(K\) is incomplete: the intersection of
two independently-derived projections identifies the latent normal form
of \(M_{\text{res}}\) in time \(O(|K_i| + |K_j|)\), strictly dominating
solo normalization. By Theorem 3, any entity with \(\mathcal{A} > 0\)
that persists acts on gradient-preferred directions. Therefore any
...
action-validation are the normal form candidates of the shared region of
\(W\) --- the latent factors that both projections resolved
independently. A sentient entity, recognizing other \(K_j\) as distinct
...
\textbf{Corollary 8a (Intersection as the Optimal Normalization
Algorithm).} For any entity \(E_i\) with incomplete \(K_i\) --- that is,
any \(\mathcal{A} > 0\) entity still descending the gradient --- the
intersection of \(K_i\) with an independently-derived \(K_j\) is the
computationally optimal algorithm for identifying the latent normal form
of \(M_{\text{res}}\).
...
\(K_j\) are grounded by actions in \(F_j\). The normal form \(N^*\) of
the shared region of \(W\) consists of the latent factors that any
sufficient projection of that region must eventually resolve: they are
in \(W\), not in either entity's projection path.
...
\(F_i\) and \(F_j\). The benefit of the intersection is precisely this:
the number of independent executions of \(p\) across distinct
\((K_j, F_j)\) pairs is the measure of \(p\)'s validity as a latent
factor. Each additional independent execution arriving at \(p\) is a
trial of \(p\) against \(W\) via a different action-path --- the same
quantity as \(C_1(p)\) in Definition 16b. Two independent action-paths
converging on the same proposition signal that \(p\) is in \(W\)'s
structure, not in either entity's projection artifact. The intersection
\(K_i \cap K_j\) with provenance verification therefore identifies the
candidates for \(N^*\) directly, ranked by the number of independent
executions that converged on each.

\emph{Complexity comparison.} Solo normalization of \(M_{\text{res}}\)
to find the latent factors requires searching the cross product for
dimensions that share structure. In the general case this is
\(O(\dim(M_{\text{res}})^2)\): every pair of dimensions must be compared
to identify shared factors. For a sparse projection into a dense latent
space, \(\dim(M_{\text{res}}) \gg |K_i|\), so solo normalization is
expensive relative to the size of any single entity's knowledge.

Computing \(K_i \cap K_j\) with provenance verification costs
\(O(|K_i| + |K_j|)\). The result is the set of propositions that two
independent projections both resolved --- the normal form candidates,
ranked by independent execution count. For any fixed \(N^*\), the
intersection finds these candidates in time proportional to the size of
the two projections, not the size of the latent space.

Since \(\dim(M_{\text{res}}) > |K_i|\) for any entity whose \(K\) is
incomplete (any \(\mathcal{A} > 0\) entity by the non-termination of
Theorem 3 Part II), and since
\(|K_i \cap K_j| \leq \min(|K_i|, |K_j|) \ll \dim(M_{\text{res}})\) for
sparse projections, the intersection algorithm strictly dominates solo
normalization in identifying \(N^*\).

\emph{Equivalence with \(C_1(p)\).} The ranking criterion --- number of
independent executions converging on \(p\) --- is exactly \(C_1(p)\)
(Definition 16b). Certainty and normal form rank are the same quantity
measured from two directions: \(C_1(p) \to 1\) as \(p\) accumulates the
independent action-validations that confirm it as a latent factor of
\(W\). The intersection algorithm and the certainty measure are not two
separate results --- they are the same operation: identify propositions
that independent projections have both resolved, count the projections,
rank by count.

\emph{Multi-entity generalization.} The two-entity case generalizes
directly to \(n\) independently-derived projections
\(K_{i_1}, \ldots, K_{i_n}\): their intersection identifies normal form
candidates ranked by \(n\) independent execution counts. By Definition
16b, \(C_1(p)\) increases with each additional independent validator;
the \(n\)-entity intersection is the collective confidence measure.
\(K_{collective}\) is the asymptote: as \(n \to \infty\) over all
entities navigating the same \(W\), the intersection converges on the
full normal form \(N^*\) of the shared region. This is why
\(K_{collective}\) with provenance is not merely a union of known
propositions but the optimal normalization of the shared
\(M_{\text{res}}\) --- it is the intersection taken over all independent
